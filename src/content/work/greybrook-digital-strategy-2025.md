---
title: "Greybrook Digital Strategy 2025"
client: "Greybrook Realty Partners"
role: "Digital marketing lead"
date: "March to December 2025"
year: 2025
summary: "Took over Greybrook's digital marketing function in March 2025 and ran it through the year. Audit, content strategy, execution across LinkedIn, Instagram, YouTube, and the firm's site, plus the mid-year performance review and revised H2 plan."
tags: ["Strategy", "Digital", "Content"]
metrics:
  - value: "+300%"
    label: "LinkedIn engagement YoY"
  - value: "+152%"
    label: "Share volume YoY"
  - value: "+164%"
    label: "Reach YoY"
cardCover: "/images/work/greybrook-logo-cover.png"
featured: true
order: 3
accent: "#232323"
accentSecondary: "#929292"
---

First step was the audit. The firm's website, LinkedIn, Instagram, and YouTube each got a separate read on where they were performing and where the existing setup was leaving opportunity on the table. Recommendations ranged from technical SEO fixes through to platform-specific content variation.

The content strategy that came out of the audit organized around three objectives: thought leadership, investor engagement, and brand awareness. Content types broke into market analysis, project spotlights, community engagement, and educational resources, with platform-specific treatments for LinkedIn, Instagram, and the website blog.

Execution ran across three workstreams. The video program ran on a near-monthly cadence, with a social-cuts layer I added that took each Boardroom Brief recording from two distribution surfaces to five. Inside Track in October was the big event of the year, with a three-phase content program built around the day. And the standard four-piece package shipped on PR Newswire whenever a deal closed.

Mid-year, I ran a Q1-vs-Q2 performance review. LinkedIn engagement rates had moved up. Instagram engagement had softened with higher posting volume. The H2 plan focused on quality over quantity, platform-native video formats, shareability-focused content like carousels and quote cards, and a tighter publishing cadence to avoid follower fatigue. Targets were specific: +150 Instagram followers and +700 LinkedIn followers, 5.0% engagement on Instagram and 12.5% on LinkedIn, and a 25% lift on combined shares.

By the year-end window I was tracking, LinkedIn engagement on Greybrook's corporate channel was up roughly 300% year-over-year, share volume up 152%, and reach up 164%.
